...
public class GridCreator extends JPanel {
	...
	private JButton randomizeShips;
	...
	public void setup() {
		...
		randomizeShips = new JButton("Randomize Grid");
		randomizeShips.setBounds(buttonXPos, 0, window.getWidth() - buttonXPos, TILE_SIZE - 5);
		...
		add(randomizeShips);
		randomizeShips.setVisible(true);
		repaint();
	 ...
	 //Gets called when right click is pressed on a ship panel.
	...
	private void placeShipPanelOnGrid(int x, int y, int shipNum, boolean isVertical) {
	 ...
	 // Checks if the show finish button should be added, and if it should be add it
